...
OpenAI::ChatGPT::ChatGPT(const std::string& token)
  : m_token{ token }, m_link{ "https://api.openai.com/v1/chat/completions" }
{
}
...
std::string OpenAI::ChatGPT::askWhisper(const std::string& audio_path)
{
  ...
  cpr::Multipart multipart_data = { { "model", model_name }, { "file", cpr::File{ audio_path } } };

  auto r = cpr::Post(cpr::Url{ url },
                     cpr::Header{ { "Authorization", auth_token }, { "Content-Type", "multipart/form-data" } },
                     multipart_data)
               .text;
  nlohmann::json j;
  try
