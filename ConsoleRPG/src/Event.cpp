...
void Event::shopEncouter(Character &character) {
  ...
  for (size_t i = 0; i < nrOfItems; i++) {
    ...
    if (coinToss > 0)
      merchantInv.addItem(
          Weapon(character.getLevel() + rand() % 5, rand() % 4));
    else
    ...
    switch (choice) {
      ...
      case 1:  // Buy
        ...
        if (choice == -1) {
          ...
          cout << "Bought item " << merchantInv[choice].getName() << " -"
               << merchantInv[choice].getBuyValue() << "\n";
      ...
      case 2:  // Sell
        ...
        if (character.getInventorySize() > 0) {
          ...
          while (cin.fail() || choice > character.getInventorySize() ||
                 choice < -1) {
            system("clear");
    ...
    if (playerTurn && !playerDefeated) {
      ...
      for (size_t i = 0; i < enemies.size(); i++) {
        ...
        combatTotal = enemies[i].getAccuracy() +
                      (character.getDefence() + character.getAddedDefence());
        enemyTotal = enemies[i].getAccuracy() / (double)combatTotal * 100;
        playerTotal = (character.getDefence() + character.getAddedDefence()) /
                      (double)combatTotal * 100;
        combatRollPlayer = rand() % playerTotal + 1;
        combatRollEnemy = rand() % enemyTotal + 1;
